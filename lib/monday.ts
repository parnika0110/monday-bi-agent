...
export async function fetchBoardWithItems(boardId: string): Promise<MondayBoard> {
  ...
  do {
    ...
    const page: MondayItemsPageResponse = await mondayFetch<MondayItemsPageResponse>(
      itemsQuery,
      cursor ? { boardId: [boardId], cursor } : { boardId: [boardId] }
    );
    ...
    for (const item of itemsPage.items) {
      items.push({
        id: item.id,
        name: item.name,
        column_values: item.column_values.map((cv: { id: string; text: string | null; type: string; column: { title: string } }) => ({
          id: cv.id,
